...
mkdir .temp/
mkdir .temp/H_catalogs/
mkdir .temp/H_tmerged/
...
df_H_extracted_transcripts <- read.table(".temp/Catalog_specificity/H_extracted_transcripts.ids",
                                       header=F, sep="\t") %>%
...
df_H_extracted_transcripts_detected <- read.table(".temp/Catalog_specificity/H_extracted_transcripts_detected.ids",
                                       header=F, sep="\t") %>%
  rename(H_intronChain_id="V1",
         H_transcript_id="V2")
...
df_H_transcripts_with_catalogs <- read.table(".temp/Catalog_specificity/H_transcripts_with_catalog_names.tsv",
                                       header=F, sep="\t") %>%
...
df_H_joined_tables %>% 
  ...
  theme(axis.text.x = element_text(size = 12, colour = "black"), #, angle = 60, hjust = 1,
        ...
        strip.text.y = element_text(size = 18)) -> catalog_specificity_plot


# Preview plot
catalog_specificity_plot
```

# Save plot - catalog specificity with composition
```{r}
pdf(file="catalog_specificity_plot.pdf", width = 8, height = 6)
catalog_specificity_plot
```
