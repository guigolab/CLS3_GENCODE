...
# DESCRIPTION
# +++++++++++++++++++++++++++++++++++++++++++++
# Takes data in gtf format from ./Data/H_catalogs/ and calculates human catalog comparison plot input
...
rm -r .temp_H/
rm H_cat_comp_plot_input.tsv
rm H_cat_comp_recount_input.tsv
echo '>> Done <<'
...
# Dir for .temp data
mkdir -p .temp_H/
mkdir .temp_H/H_catalogs/
mkdir .temp_H/Cage/
mkdir .temp_H/PolyA/
mkdir .temp_H/Fl/
mkdir .temp_H/Decompressed/
mkdir .temp_H/Recount_support/
echo '>> Done <<'
...
##########################################################################################
##########################################################################################
# Generate data for Human comparison plot
##########################################################################################
##########################################################################################
...
for file in `ls ./H_catalogs/*gtf.gz | awk -F'/' '{print $NF}' | sed 's/.gtf.gz//g'`
    do
        ...
        echo -e "$file\t$propFl\t$gene\t$nbTx" >> ../H_cat_comp_plot_input.tsv
...
echo -e "catalogue\ttotal\tsupported\tunsupported\tsupported_percentage" > ../H_cat_comp_recount_input.tsv
for file in `ls ./H_catalogs/*gtf.gz | awk -F'/' '{print $NF}' | sed 's/.gtf.gz//g'`
  do
      echo "Name: $file"
      ...
      # Save results to tsv
      echo -e "$file\t$total_tx\t$supported_tx\t$not_supported_tx\t$supported_tx_percentage" >> ../H_cat_comp_recount_input.tsv
      echo "+++ Done +++"
  done
...
df <- read.table("H_cat_comp_plot_input.tsv", header = F, as.is = T, sep = "\t") %>%
  rename(
    Catalogue = "V1",
    fl = "V2",
    gene = "V3",
...
# PIE-CHARTS <- Human catalog comparison plot
```{r}
cbPalette <- c(
...
df_recount <- read.table("H_cat_comp_recount_input.tsv", header = T, as.is = T, sep = "\t") %>%
...
# Save plot
```{r}
pdf("H_catalog_comparison.pdf", bg = "white", width = 14, height = 9)
plot_H_cat_comparison
```
