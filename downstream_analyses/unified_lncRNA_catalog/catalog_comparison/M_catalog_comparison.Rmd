...
# DESCRIPTION
# +++++++++++++++++++++++++++++++++++++++++++++
# Takes data in gtf format from ./Data/M_catalogs/ and calculates mouse catalog comparison plot input
...
rm -r .temp_M/
rm M_cat_comp_plot_input.tsv
rm M_cat_comp_recount_input.tsv
echo '>> Done <<'
...
# Dir for .temp data
mkdir -p .temp_M/
mkdir .temp_M/M_catalogs/
mkdir .temp_M/Cage/
mkdir .temp_M/PolyA/
mkdir .temp_M/Fl/
mkdir .temp_M/Decompressed/
mkdir .temp_M/Recount_support/
echo '>> Done <<'
...
for file in `ls ./M_catalogs/*gtf.gz | awk -F'/' '{print $NF}' | sed 's/.gtf.gz//g'`
    do
        ...
        echo -e "$file\t$propFl\t$gene\t$nbTx" >> ../M_cat_comp_plot_input.tsv
...
echo -e "catalogue\ttotal\tsupported\tunsupported\tsupported_percentage" > ../M_cat_comp_recount_input.tsv
for file in `ls ./M_catalogs/*gtf.gz | awk -F'/' '{print $NF}' | sed 's/.gtf.gz//g'`
  do
      echo "Name: $file"
      ...
      # Save results to tsv
      echo -e "$file\t$total_tx\t$supported_tx\t$not_supported_tx\t$supported_tx_percentage" >> ../M_cat_comp_recount_input.tsv
      echo "+++ Done +++"
  done
...
df <- read.table("M_cat_comp_plot_input.tsv", header = F, as.is = T, sep = "\t") %>%
  rename(
    Catalogue = "V1",
    fl = "V2",
    gene = "V3",
...
# PIE-CHARTS <- Mouse catalog comparison plot
```{r}
...
df_recount <- read.table("M_cat_comp_recount_input.tsv", header=T, as.is=T, sep="\t") %>%
...
# Save plot
```{r}
pdf("M_catalog_comparison.pdf", bg = "white", width = 14, height = 9)
plot_M_cat_comparison
```
