...
# Remove old results
rm -r .temp_H/
rm H_IC_analysis_targets_input.tsv
rm H_IC_analysis_recount_input.tsv
...
while read file || [ -n "$file" ]
  do
    ...
    echo -e "$target_category\t$name\t$all_IC\t$all_TX\t$targeted_IC\t$targeted_TX\t$detected_genuine_IC\t$detected_genuine_IC_GENCODE\t$NOTdetected_genuine_IC\t$detected_genuine_TX\t$detected_genuine_TX_GENCODE\t$NOTdetected_genuine_TX" >> ../H_IC_analysis_targets_input.tsv
    echo "++ Done ++"
...
while read file || [ -n "$file" ]
  do
    ...
    echo "~~~ Preparing introns ~~~"
      ...
      # Save results to tsv
      echo -e "$name\t$total_tx\t$supported_tx\t$not_supported_tx\t$supported_tx_percentage" >> ../../H_IC_analysis_recount_input.tsv
      echo "+++ Done +++"    
...
df_H_intronChains_target_detection <- read.table("H_IC_analysis_targets_input.tsv", header=F, sep="\t") %>%
...
# Read recount data
df_recount_support <- read.table("H_IC_analysis_recount_input.tsv", header=F, sep="\t") %>%
...
figure_s11a_input %>% 
  ...
  facet_grid2(cols=vars(factor(H_target_category, levels=c("merged", "individual lncRNA catalogs",
              ...
              space = "free",
              shrink = TRUE,
              scales = "free") -> plot_H_IC_analysis
...
# Save as pdf
```{r}
pdf(file="H_intron_chain_analysis.pdf", width = 18, height = 6)
plot_H_IC_analysis
```
