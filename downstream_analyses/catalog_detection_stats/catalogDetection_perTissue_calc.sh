...
#---header
	echo -e "spec\tclass\tregionType\tdetectedRegions\tallRegions\tdetectedTMs\tdetectedICs\tdetectedLoci\ttissue\tpropDetectedRegions" > stats/${spec}.MergedTargetRegions.proportionDetected_perTissue
	...
	while read tissue; do
		...
		do
			...
			currentDetected=`grep $tissue data/masterTable/${spec}_masterTable_refined.gtf | grep -w transcript | eval "$targets" | awk -F"\"" '{print $6}' | grep . | perl -pi -e "s/,/\n/g" | eval "$get" | sort --parallel=4 -T /tmp/ | uniq | grep -Ff - data/targets/${spec}_CLS3_targetDesign_mergedRegions.gtf | wc -l`
			...
			echo -e "$spec\t$class\tdetectedNonNovel\t$currentDetectedNonNovel_regions\t$currentTotalRegions\t0\t$currentDetectedNonNovelICs\t0\t$tissue"|awk '{print $0"\t"($4/$5)*100}'>> stats/$spec\.MergedTargetRegions.proportionDetected.perTissue_${tissue}
			echo -e "$spec\t$class\tdetectedNovel\t$currentDetectedNovel_regions\t$currentTotalRegions\t0\t$currentDetectedNovelICs\t0\t$tissue"|awk '{print $0"\t"($4/$5)*100}'>> stats/$spec\.MergedTargetRegions.proportionDetected.perTissue_${tissue}
			echo -e "$spec\t$class\tdetectedALL\t$currentDetected\t$currentTotalRegions\t0\t$currentDetectedICs\t0\t$tissue"|awk '{print $0"\t"($4/$5)*100}'>> stats/$spec\.MergedTargetRegions.proportionDetected.perTissue_${tissue}
		done
	done < data/metadata/${spec}_samples
done
